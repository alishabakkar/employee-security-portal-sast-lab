...
const USERS = [
  { username: "admin", password: "password123", role: "admin", email: "admin@codelab.local" },
  { username: "jane", password: "welcome1", role: "hr", email: "jane@codelab.local" },
  { username: "ops", password: "opsops", role: "ops", email: "ops@codelab.local" }
];
...
jwtSecret: "[REDACTED]",
  ...
  fetch('settings.json')
    .then(r => r.json())
    .then(settings => {
      document.getElementById('configDump').textContent =
    JSON.stringify({ ...dump, settings }, null, 2);
    })
    .catch(() => {
     document.getElementById('configDump').textContent =
